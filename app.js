...
function deleteSelectedCards() {
  if (confirm(`آیا از حذف ${selectedCarIds.size} مورد مطمئن هستید؟`)) {
    carsData = carsData.filter(c => !selectedCarIds.has(c.id));
    saveToStorage();
    selectedCarIds.clear();
    updateBatchBar();
    handleSearch();
...
function saveCar(e) {
  ...
  if (editingCarId) {
    const idx = carsData.findIndex(c => c.id === editingCarId);
    if (idx !== -1) carsData[idx] = { id: editingCarId, name: carName, belts };
    showToast('ویرایش انجام شد');
  } else {
    carsData.push({ id: Date.now().toString(), name: carName, belts });
...
function saveToStorage() {
  localStorage.setItem('beltfind_data', JSON.stringify(carsData));
}
